...
def test_load_urls(setup_test_environment):
    ...
    assert len(setup_test_environment.urls) > 0, "URLs should be loaded from the input CSV"
...
def test_write_output(setup_test_environment):
    ...
    assert os.path.exists(setup_test_environment.output_file_path), "CSV file should be created"

    with open(setup_test_environment.output_file_path, mode='r', encoding='utf-8') as file:
